...
test( ".destroy() - class removal", function(){
	...
	domEqual( "#widget", function(){
		$( "#widget" ).classesWidget()
			.classesWidget( "toggleClasses", false )
			.classesWidget( "toggleClasses", true )
			.classesWidget( "removeClasses" )
			.classesWidget( "toggleClasses", true )
			.classesWidget( "destroy" );
	});

});

test( "._add/_remove_/toggleClass()", function(){
	expect( 24 );

	var widget = $( "#widget" ).classesWidget();

	testElements( true, "add" );
	widget.classesWidget( "toggleClasses", false );
	testElements( false, "toggle", ", false ");
	widget.classesWidget( "toggleClasses", true );
	testElements( true, "toggle", ", true ");
	widget.classesWidget( "removeClasses" );
	testElements( false, "remove" );

	function testElements( bool, method, toggle ) {
		toggle = toggle || "";
